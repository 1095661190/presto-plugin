...
public class LookupService
{
    ...
    public static final Map<String, String> countryMap = new HashMap<>(countryCode.length);
    ...
    public LookupService(RandomAccessFile databaseFile) throws IOException {
    ...
    private void init() throws IOException {
        ...
        if ((dboptions & GEOIP_CHECK_CACHE) != 0) {
            mtime = 0;//20200506由原有mtime = databaseFile.lastupdateTime();修改而来
        }
    ...
    synchronized void _check_mtime() {
        if (1 == 1) {//20200506改，因为可以读流，所以不再考虑数据文件更迭的问题
            return;
        }
        try {
            if ((dboptions & GEOIP_CHECK_CACHE) != 0) {
